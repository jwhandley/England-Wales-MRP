...
survey_cols <- c("wt_new_W19_result",
                 "ageW19",
...
survey <- read_stata("data/BES2019_W24_Panel_v24.0.dta", col_select = all_of(survey_cols)) %>%
  filter(wave19 == 1,
         !is.na(genElecTurnoutRetroW19)) %>%
  mutate(sex = as_factor(gender),
         ethnicity = case_match(p_ethnicityW19,
                                1 ~ "White British",
                                2 ~ "White Other",
                                c(7:10,14) ~ "Asian",
                                11:13 ~ "Black",
                                .default = "Other"),
         ethnicity = factor(ethnicity, levels = c("White British",
                                                  "White Other",
                                                  "Black",
                                                  "Asian",
                                                  "Other")),
         degree = ifelse(p_edlevelW19 >= 4,"Degree","No degree"),
         degree = factor(degree, levels = c("Degree", "No degree")),
         age = case_match(ageW19,
         ...
         age = factor(age, levels = c("Aged 16 to 24 years",
                                      ...
                                      "Aged 50 to 64 years",
                                      "Aged 65 years and over")),
         turnout = ifelse(genElecTurnoutRetroW19 == 1, "Voted", "Didn't vote"),
         vote = case_match(generalElectionVoteW19,
                           1 ~ "Conservative",
                           2 ~ "Labour",
                           3 ~ "Liberal Democrat",
                           0 ~ NA,
                           ...
                           NA ~ NA,
                           .default = "Other"),
         vote = factor(vote, levels = c("Conservative","Labour","Liberal Democrat","Other")),
         turnout_vote = coalesce(vote,turnout),
         turnout_vote = ifelse(turnout_vote == "Voted", "Didn't vote", turnout_vote),
         turnout_vote = factor(turnout_vote, levels = c("Conservative","Labour","Liberal Democrat","Other","Didn't vote")),
         wt = wt_new_W19_result,
         pcon_code = pcon_codeW19) %>%
  select(wt,turnout,vote,turnout_vote,age,sex,degree,pcon_code) %>%
  left_join(context) %>%
  filter(!if_any(c(turnout_vote,age,sex,degree,pcon_code), ~is.na(.x))) %>%
  mutate(LD19 = ifelse(is.na(LD19), 0, LD19),
...
fit <- brm(turnout_vote ~ Turnout19 + Con19 + Lab19 + LD19 + degree + sex + (1|age) + (1|pcon_code),
...
## Save fitted model
saveRDS(fit,"output/2019_val_fit.RDS")
...
pstrat %>%
  ...
  labs(x = "2019 general election result",
       y = "MRP prediction",
       title = "Cross-validation of basic MRP for the 2019 UK general election",
       subtitle = "BES Internet Panel Wave 19; 2021 England and Wales census",
       caption = "@jwhandley17")

ggsave("output/2019_val_scatter.png", width = 8, height = 6)
